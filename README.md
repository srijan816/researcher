...
> **Models:** This app is extensively tested with **MiniMax M3**, and that is the
> recommended default. The backend talks to models over the Anthropic-compatible
> API, so other models work too — see [Using other models](#using-other-models).

## Quick Start

You need [Docker](https://docs.docker.com/get-docker/) (with the Compose plugin)
and a **MiniMax API key** from <https://www.minimax.io/>. That is the only
required secret — the stack bundles its own search engine and database.

```bash
# 1. Clone
git clone https://github.com/srijan816/researcher.git
cd researcher

# 2. Add your key
cp deploy/.env.example deploy/.env
#    then edit deploy/.env and set:  MINIMAX_API_KEY=your_key_here

# 3. Launch (builds images on first run — a few minutes)
./scripts/quickstart.sh
```

Then open **<http://localhost:3000>** and ask a research question.

That is it. `quickstart.sh` checks Docker, creates `deploy/.env` if missing,
verifies your key is set, builds the stack, and waits for the backend to report
healthy. Prefer the raw command? It is:
...
```bash
# Logs
docker compose -f deploy/compose/docker-compose.yaml logs -f aiq-agent
# Stop (keeps data)
docker compose -f deploy/compose/docker-compose.yaml down
# Stop and wipe all data (jobs, checkpoints, scrape cache)
docker compose -f deploy/compose/docker-compose.yaml down -v
```

**What you get out of the box:** the Next.js UI, the FastAPI backend on MiniMax
M3, a bundled SearXNG metasearch engine, and a PostgreSQL store — all wired
together, no auth, single-user. Optional keys (`NVIDIA_API_KEY` for reranking,
`EXA_API_KEY` for premium search) are documented in `deploy/.env.example` and
improve quality but are never required.

**Requirements:** Docker with ~8 GB RAM allocated and ~15 GB free disk for the
images and data volumes. First build pulls a Python 3.13 base and compiles the
workspace, so expect a few minutes; subsequent starts are fast.
...
MiniMax M3 is the tested, recommended default, but nothing in the pipeline is
hard-wired to it. Every agent role reads its model from the config, and the
backend speaks the Anthropic-compatible API, so swapping models is configuration,
not code.

**Try a different MiniMax model** — set the per-role overrides in `deploy/.env`
(all default to `MiniMax-M3`):
...
**Point at a different Anthropic-compatible provider** — edit the `base_url`,
`api_key`, and `model_name` fields of the `llms:` profiles in
`configs/config_cli_minimax_ddgs.yml`. Each profile is a standard NAT `anthropic`
LLM block, so any endpoint that implements `/v1/messages` (including Anthropic
itself) drops in.

Quality and tool-call reliability vary by model — the prompts, thinking budgets,
and timeouts in the config were tuned against MiniMax M3, so expect to re-tune
`temperature`, `max_tokens`, and the `thinking` blocks when switching families.
...
For normal use, the [Quick Start](#quick-start) Docker flow is the recommended
path — it needs nothing on your machine but Docker and a MiniMax key.

If you are **developing the code** and want to run the backend from source (hot
reloads, debugging, test runs), install the Python workspace with
[uv](https://docs.astral.sh/uv/):

```bash
# Install dependencies into a local .venv (Python 3.13)
uv sync

# Export your keys (or source them from deploy/.env)
export MINIMAX_API_KEY=your_key_here

# Run the test suite to confirm the workspace is healthy
uv run pytest
```

You still need a running SearXNG for live web search. The simplest option is to
start just that one service from the bundled compose stack:

```bash
docker compose -f deploy/compose/docker-compose.yaml --env-file deploy/.env up -d searxng
```

Then point the backend at it (`SEARXNG_URL=http://localhost:8080`) and launch the
server with the MiniMax config in `configs/config_cli_minimax_ddgs.yml`. The
frontend lives in `frontends/ui` and runs with the standard Next.js dev server
(`npm install && npm run dev` from that directory).
...
The full, commented list of environment variables lives in
[`deploy/.env.example`](deploy/.env.example) — that file is the source of truth.
The primary backend config is `configs/config_cli_minimax_ddgs.yml`.

The variables that matter most:

| Variable | Required | Purpose |
| --- | --- | --- |
| `MINIMAX_API_KEY` | **Yes** | MiniMax API credential — the only required secret. |
| `NVIDIA_API_KEY` | No | Enables cross-encoder reranking + corpus embeddings (falls back to heuristic + BM25 without it). |
| `EXA_API_KEY` | No | Adds Exa as a premium search provider. |
| `MINIMAX_*_MODEL` | No | Per-role model overrides (default `MiniMax-M3`). See [Using other models](#using-other-models). |
| `BACKEND_CONFIG` | No | Backend config file path (default: the MiniMax config). |
| `FRONTEND_PORT` / `PORT` | No | UI / API ports (default `3000` / `8000`). |
| `REQUIRE_AUTH` | No | `false` by default (single-user, no login). |
| `SEARXNG_URL` | No | SearXNG endpoint (default: the bundled container). |
| `NAT_JOB_STORE_DB_URL` | No | Job-store database URL (default: bundled Postgres). |

Configuration is layered: NAT YAML config (`configs/`), the Compose environment
(`deploy/.env`), and FastAPI/frontend settings. For a standard local run you only
ever touch `deploy/.env`.
