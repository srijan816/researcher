...
@lru_cache(maxsize=4)
def _load_rows(corpus_jsonl: str) -> dict[str, Any]:
    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            ...
            # Precompute tokens and counts for BM25
            row["_tokens"] = _tokenize(row["_search_text"])
            row["_token_counts"] = Counter(row["_tokens"])
            row["_doc_len"] = len(row["_tokens"])
            rows.append(row)
    ...
    N = len(rows)
    if N == 0:
        return {"rows": (), "avgdl": 1.0, "idf": {}}

    total_len = sum(row["_doc_len"] for row in rows)
...
def search_rows(
    ...
    *,
    ...
    corpus_data = _load_rows(corpus_jsonl)
    rows = corpus_data["rows"]
    avgdl = corpus_data["avgdl"]
    idf = corpus_data["idf"]
    ...
    for row in rows:
        ...
        doc_len = row["_doc_len"]

        score = 0.0
        for term, weight in term_counts.items():
            tf = row["_token_counts"].get(term, 0)
            if tf > 0:
                # BM25 term score
                term_idf = idf.get(term, 0.0)
                numerator = tf * (k1 + 1)
                denominator = tf + k1 * (1.0 - b + b * (doc_len / avgdl))
                term_score = term_idf * (numerator / denominator) * weight
