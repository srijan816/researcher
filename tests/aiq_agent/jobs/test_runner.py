...
class TestSubmitDeepResearchJob:
    ...
    async def test_submit_agent_job_passes_webhook_config(self):
        """Test submit_agent_job forwards terminal webhook config into worker args."""
        from aiq_api.jobs.submit import submit_agent_job
    ...
    @pytest.mark.asyncio
    async def test_submit_agent_job_passes_include_images(self):
        """Test submit_agent_job forwards the include_images flag into worker args."""
        from aiq_api.jobs.submit import submit_agent_job
    ...
    async def test_submit_agent_job_defaults_include_images_false(self):
        """Test include_images defaults to False in worker args when not provided."""
        from aiq_api.jobs.submit import submit_agent_job
...
class TestEventStore:
    ...
    def test_job_events_channel_handles_long_job_ids(self):
        """PostgreSQL NOTIFY channels must stay under the 63-byte identifier limit."""
        from aiq_api.jobs.event_store import get_job_events_channel

        job_id = "skill-craft-v2-medium-g5-6-psd-i-u13-l1-vouy1f-retry-mq4oywx3"
        channel = get_job_events_channel(job_id)

        assert channel.startswith("job_events_")
        assert len(channel.encode("utf-8")) <= 63
        assert channel == get_job_events_channel(job_id)
        assert channel != get_job_events_channel(f"{job_id}-next")
