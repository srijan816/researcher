...
from aiq_agent.common.fact_ledger_store import fact_ledger_enabled
...
from .adversarial_verifier import verifier_enabled
...
from .gapfill import gapfill_snapshot
...
class DeepResearcherAgent:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            verifier_llm: Optional dedicated adversarial claim-verifier LLM.
        ...
        self.job_id = job_id
        self._verifier_llm = verifier_llm
        ...
        def get_research_progress_snapshot() -> str:
            """Return live budget, source, structured-artifact, and gap-fill progress.

            Call this between researcher batches and before final synthesis.
            The `gap_fill` block reports targeted gap-fill rounds used vs
            allowed and the remaining reserve search budget; the `sections`
            block reports planned report sections that have no section brief
            yet. Use it to decide whether to launch another targeted
            gap-filling task, move to synthesis, or acknowledge limitations.
            """
            ...
            budget_snapshot = get_session_budget_snapshot()
            try:
                gap_fill = gapfill_snapshot(
                    tier=self._active_research_depth,
                    rounds_used=max(0, self._dossier_compile_count - 1),
                    budget_snapshot=budget_snapshot,
                    budget_profile=self._active_budget_profile,
                )
            except Exception:
                logger.debug("Unable to compute gap-fill snapshot", exc_info=True)
                gap_fill = {}
            try:
                plan_content = self._read_backend_file(backend, "/shared/plan.json")
                brief_paths = self._backend_glob_paths(backend, "/shared/section_briefs/*.md")
                sections = self._section_coverage(plan_content, brief_paths)
            except Exception:
                logger.debug("Unable to compute section coverage", exc_info=True)
                sections = {}
            snapshot = {
                ...
                "artifacts": {
                    ...
                    "verification_report_present": _read_present(VERIFICATION_REPORT_PATH),
                },
            }
            return json.dumps(snapshot, indent=2, ensure_ascii=False)

        @tool
        async def compile_research_dossier() -> str:
            ...
            use as the truth substrate. The runtime also adversarially verifies
            high-risk claims and writes `/shared/verification_report.json`.
            """
            self._dossier_compile_count += 1
            ...
            try:
                ...
                source_count = 0
            # Deterministic backend step: adversarially verify high-risk claims
            # now that the dossier exists, before final synthesis. Fail-open.
            verification_block = ""
            try:
                verification_summary = await self._run_adversarial_verification_live(files)
                if verification_summary:
                    verification_block = (
                        "## Adversarial Verification Summary\n"
                        f"{json.dumps(verification_summary, sort_keys=True)}\n"
                        "Full per-claim verdicts: /shared/verification_report.json. Contradicted claims were "
                        "downgraded in /shared/claim_table.json and must be dropped or explicitly hedged naming "
                        "the contradiction. Numeric/date claims with verdict not_addressed must be hedged "
                        '("reported", "according to X").\n\n'
                    )
            except Exception:
                logger.warning("Adversarial verification failed open during dossier compile", exc_info=True)
    ...
    def _resolve_verifier_llm(self) -> Any | None:
        """Resolve the adversarial-verifier LLM.

        A dedicated ``verifier_llm`` (wired through the builder config) wins.
        Otherwise fall back to the active tier's researcher-path LLM, which is
        already a fast non-thinking profile. Fail-open: returns None when no
        LLM can be resolved.
        """
        if self._verifier_llm is not None:
            return self._verifier_llm
        try:
            depth = self._active_research_depth
            if depth == "deep" and self.llm_provider.has_role(LLMRole.DEEP_RESEARCHER):
                return self.llm_provider.get(LLMRole.DEEP_RESEARCHER)
            if depth == "deeper" and self.llm_provider.has_role(LLMRole.DEEPER_RESEARCHER):
                return self.llm_provider.get(LLMRole.DEEPER_RESEARCHER)
            return self.llm_provider.get(LLMRole.RESEARCHER)
        except Exception:  # noqa: BLE001 - fail-open by design
            logger.debug("No adversarial-verifier LLM available", exc_info=True)
            return None

    async def _run_adversarial_verification_live(self, files: dict[str, Any]) -> dict[str, int] | None:
        """Adversarially verify high-risk claims against the live virtual state.

        Runs as deterministic backend Python inside the dossier-compile step,
        before final synthesis. Writes `/shared/verification_report.json` and
        downgrades contradicted claims in `/shared/claim_table.json`.
        Fail-open: returns None whenever verification cannot run.
        """
        if not verifier_enabled() or self._active_research_depth == "shallow":
            return None
        llm = self._resolve_verifier_llm()
        if llm is None:
            return None

        claim_table_content = str(files.get("/shared/claim_table.json") or "").strip()
        if not claim_table_content:
            # The canonical merge normally happens post-run; merge fragments
            # live so verification (and Step 5b) can use the canonical table.
            fragments = [
                str(value or "").strip()
                for path, value in files.items()
                if self._is_claim_fragment_path(str(path)) and str(value or "").strip()
            ]
            if fragments:
                table, _errors = merge_claim_tables_json(fragments)
                if table is not None:
                    if self.job_id:
                        table.job_id = self.job_id
                    claim_table_content = table.model_dump_json(indent=2)
                    self._upsert_live_virtual_file("/shared/claim_table.json", claim_table_content)
        if not claim_table_content:
            return None

        evidence_packet_content = str(files.get("/shared/evidence_packet.json") or "").strip() or None
        outcome = await run_adversarial_verification(
            llm=llm,
            claim_table_content=claim_table_content,
            evidence_packet_content=evidence_packet_content,
            tier=self._active_research_depth,
        )
        if outcome is None:
            return None
        self._upsert_live_virtual_file(VERIFICATION_REPORT_PATH, outcome.report_json)
        if outcome.updated_claim_table_json:
            self._upsert_live_virtual_file("/shared/claim_table.json", outcome.updated_claim_table_json)
        return outcome.summary

    async def _ensure_adversarial_verification_in_result(self, result: dict | Any) -> None:
        """Post-run fallback: verify claims when the live pass never ran.

        The primary verification pass runs inside `compile_research_dossier`.
        If the model skipped that tool, this deterministic fallback still
        produces `/shared/verification_report.json` and downgrades
        contradicted claims for the post-run gates and audit artifacts.
        ...
        try:
            ...
            existing = self._coerce_file_content(
                files.get(VERIFICATION_REPORT_PATH) or files.get(VERIFICATION_REPORT_PATH.lstrip("/"))
            ).strip()
            if existing:
                return
            claim_table_content = self._coerce_file_content(
                files.get("/shared/claim_table.json") or files.get("shared/claim_table.json")
            ).strip()
            if not claim_table_content:
                return
            outcome = await run_adversarial_verification(
                llm=self._resolve_verifier_llm(),
                claim_table_content=claim_table_content,
                evidence_packet_content=self._evidence_packet_content(result) or None,
                tier=self._active_research_depth,
            )
            if outcome is None:
                return
            entry = self._file_state_entry(outcome.report_json)
            files[VERIFICATION_REPORT_PATH] = entry
            files[VERIFICATION_REPORT_PATH.lstrip("/")] = entry
            if outcome.updated_claim_table_json:
                table_entry = self._file_state_entry(outcome.updated_claim_table_json)
                files["/shared/claim_table.json"] = table_entry
                files["shared/claim_table.json"] = table_entry
            self._set_files(result, files)
            logger.info("Adversarial verification fallback wrote %s", VERIFICATION_REPORT_PATH)
        except Exception:  # noqa: BLE001 - fail-open by design
            logger.warning("Adversarial verification fallback failed open", exc_info=True)

    @staticmethod
    def _section_coverage(plan_content: str, brief_paths: list[str]) -> dict[str, Any]:
        """Best-effort planned-section vs section-brief coverage report."""
        titles: list[str] = []
        try:
            payload = json.loads(plan_content) if plan_content else {}
            if not isinstance(payload, dict):
                return {}
            toc = payload.get("report_toc") or payload.get("toc") or payload.get("sections") or []
            for item in toc if isinstance(toc, list) else []:
                title = str(item.get("title") or "").strip() if isinstance(item, dict) else str(item).strip()
                if title:
                    titles.append(title)
        except Exception:
            return {}
        if not titles:
            return {}
        brief_text = " ".join(
            Path(str(path)).stem.lower().replace("_", " ").replace("-", " ") for path in brief_paths
        )
        uncovered: list[str] = []
        for title in titles:
            tokens = re.findall(r"[a-z0-9]{4,}", title.lower())
            if tokens and not any(token in brief_text for token in tokens):
                uncovered.append(title)
        return {
            "planned_sections": titles,
            "section_brief_files": [str(path) for path in brief_paths],
            "possibly_uncovered_sections": uncovered,
            "note": "Heuristic filename-token match; confirm against section brief contents before relaunching.",
        }

    def _inject_prior_verified_facts(self, state: DeepResearchAgentState) -> DeepResearchAgentState:
        """Prime planner context with previously verified cross-run facts."""
        try:
            if not fact_ledger_enabled():
                return state
            query = self._query_without_context(self._latest_user_text(state))
            block = lookup_prior_facts_block(query)
            if not block:
                return state
            clarifier = str(state.clarifier_result or "").rstrip()
            merged = f"{clarifier}\n\n{block}" if clarifier else block
            logger.info("Injected prior-verified-facts block (%d chars) into planner context", len(block))
            return state.model_copy(update={"clarifier_result": merged})
        except Exception:  # noqa: BLE001 - fail-open by design
            logger.debug("Prior verified fact injection skipped", exc_info=True)
            return state

    def _persist_fact_ledger_store(self, result: dict | Any) -> None:
        """Persist the merged fact ledger to the cross-run SQLite store."""
        try:
            if not fact_ledger_enabled():
                return
            files = self._extract_files(result)
            content = self._coerce_file_content(
                files.get("/shared/fact_ledger.json") or files.get("shared/fact_ledger.json")
            ).strip()
            if not content:
                return
            persist_fact_ledger_json(content, job_id=self.job_id)
        except Exception:  # noqa: BLE001 - fail-open by design
            logger.debug("Fact ledger persistence skipped", exc_info=True)
    ...
    async def run(self, state: DeepResearchAgentState) -> DeepResearchAgentState:
        ...
        state = self._inject_prior_verified_facts(state)
        ...
        except Exception:  # noqa: BLE001 - snapshot enrichment only
            logger.debug("Unable to compute active budget profile", exc_info=True)
            self._active_budget_profile = {}
        if messages:
        ...
        try:
            ...
            if result:
                self._merge_structured_research_artifacts_into_result(result)
                # Fallback adversarial verification when the live (dossier
                # compile) pass never ran. Fail-open, never raises.
                await self._ensure_adversarial_verification_in_result(result)
            ...
            if result:
                ...
                self._persist_fact_ledger_store(result)
            logger.info(
