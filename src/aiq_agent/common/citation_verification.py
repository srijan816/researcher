...
# ---------------------------------------------------------------------------
# Aggressive canonicalization for fuzzy URL matching
# ---------------------------------------------------------------------------

# Tracking params dropped during aggressive canonicalization (in addition to
# every ``utm_*`` parameter). These never change which document a URL points
# to, so two URLs differing only by these are the same source.
_FUZZY_TRACKING_PARAMS = frozenset(
    {
        "fbclid",
        "gclid",
        "ref",
...
# Common second-level public-suffix labels (example.co.uk → example.co.uk).
_SECOND_LEVEL_TLD_LABELS = frozenset({"co", "com", "net", "org", "gov", "ac", "edu"})

_FUZZY_PATH_SIMILARITY = 0.85


def _strip_host_prefixes(host: str) -> str:
    """Strip leading www./m./mobile./amp. labels from a lowercase host."""
    changed = True
    while changed:
        changed = False
...
def _registered_host(host: str) -> str:
    """Approximate the registered domain (example.com, example.co.uk)."""
    labels = [label for label in host.split(".") if label]
    if len(labels) >= 3 and labels[-2] in _SECOND_LEVEL_TLD_LABELS and len(labels[-1]) <= 3:
...
def _canonical_url_parts(url: str) -> tuple[str, str, str]:
    """Return (host, path, query) after aggressive canonicalization.

    Lowercases everything, strips the scheme, leading www./m./mobile./amp.
    host labels, trailing slashes, fragments, tracking params (``utm_*``,
    fbclid, gclid, ref, ref_src, ...), and trailing ``/amp`` path segments.
    """
    url = unescape(str(url or "")).strip()
    if url and "://" not in url:
        url = f"https://{url}"
    parsed = urlparse(url)
    host = _strip_host_prefixes(parsed.netloc.lower().split(":")[0].strip())
    ...
    filtered = {
        ...
        and k.lower() not in _FUZZY_TRACKING_PARAMS
        and not k.lower().startswith("utm_")
    }
    query_str = "&".join(f"{k.lower()}={v[0]}" for k, v in sorted(filtered.items()) if v)
    return host, path, query_str


def _canonical_url(url: str) -> str:
    """Aggressively canonical form used as the primary fuzzy match key."""
    host, path, query = _canonical_url_parts(url)
...
def _canonical_paths_match(path_a: str, path_b: str) -> bool:
    """True when one canonical path is a segment-prefix of the other, or the
    two paths are highly similar (ratio >= 0.85).

    Prefix matches require the shorter path to have at least two segments so a
    bare domain (or one shallow segment) never swallows every deeper URL on
    the same host.
    """
    if path_a == path_b:
        return True
    shorter, longer = sorted((path_a, path_b), key=len)
    shorter_segments = [segment for segment in shorter.split("/") if segment]
    if len(shorter_segments) >= 2 and longer.startswith(shorter.rstrip("/") + "/"):
        return True
    if path_a and path_b:
        return difflib.SequenceMatcher(None, path_a, path_b).ratio() >= _FUZZY_PATH_SIMILARITY
    return False
...
class SourceRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._canonical_urls: dict[str, list[SourceEntry]] = {}
        self._canonical_hosts: list[tuple[str, str, str, str, SourceEntry]] = []
    ...
    def add(self, entry: SourceEntry) -> None:
        ...
        if entry.url:
            ...
            if normalized not in self._urls:
                ...
                canonical_host, canonical_path, canonical_query = _canonical_url_parts(raw)
                if canonical_host:
                    self._canonical_urls.setdefault(_canonical_url(raw), []).append(entry)
                    self._canonical_hosts.append(
                        (canonical_host, _registered_host(canonical_host), canonical_path, canonical_query, entry)
                    )
                added = True
            if raw != normalized:
    ...
    def resolve_url(self, url: str) -> str | None:
        ...
        6. Canonical — aggressive canonicalization (scheme/www/m/amp prefixes,
           fragments, tracking params, trailing /amp) matches exactly one entry
        7. Fuzzy host+path — same registered host and the canonical paths are
           segment-prefixes of each other or >= 0.85 similar
        ...
        if len(child_path_candidates) == 1:
            matched = child_path_candidates[0]
            # Sibling guard: when other registry entries share the matched
            # entry's parent directory (e.g. /story/one and /story/two), the
            # host serves distinct documents per path node, so a deeper cited
            # URL is its own document — mapping it onto the parent risks a
            # wrong citation. Reject as ambiguous.
            matched_parent = matched.path.rstrip("/").rsplit("/", 1)[0]
            siblings = [
                p
                for p in same_host
                if p.entry.url != matched.entry.url
                and p.path.rstrip("/") != matched.path.rstrip("/")
                and p.path.rstrip("/").rsplit("/", 1)[0] == matched_parent
            ]
            if siblings:
                logger.debug(
                    "[CitationVerify] Child-path match for '%s' rejected — %d sibling path(s) under '%s'",
                    url,
                    len(siblings),
                    matched_parent,
                )
                return None
            logger.debug("[CitationVerify] child-path match: '%s' → '%s'", url, matched.entry.url)
            return matched.entry.url
        if len(child_path_candidates) > 1:
            logger.debug(
                "[CitationVerify] Ambiguous child-path match for '%s' — %d candidates, rejecting",
                url,
                len(child_path_candidates),
            )
            return None
        ...
        # 6. Canonical match — trivial URL variants (scheme, www./m./mobile./amp.
        #    prefixes, fragments, utm_*/fbclid/gclid/ref tracking params,
        #    trailing slash or /amp) must not get a valid citation deleted.
        canonical = _canonical_url(url)
        canonical_entries = self._canonical_urls.get(canonical, [])
        result = self._pick_unique(
            list({e.url: e for e in canonical_entries if e.url}.values()),
            "canonical",
            url,
        )
        if result:
        ...
        # 7. Fuzzy host+path fallback — same registered host with canonical
        #    paths that are segment-prefixes of each other or highly similar.
        #    A non-empty report query must match the candidate query exactly;
        #    differing query values (e.g. ?id=123 vs ?id=999) are different
        #    documents and must not fuzzy-match.
        report_host, report_path, report_query = _canonical_url_parts(url)
        if report_host:
            report_registered = _registered_host(report_host)
            fuzzy_entries = [
                entry
                for _host, registered, candidate_path, candidate_query, entry in self._canonical_hosts
                if registered == report_registered
                and (not report_query or report_query == candidate_query)
                and _canonical_paths_match(report_path, candidate_path)
            ]
            result = self._pick_unique(
                list({e.url: e for e in fuzzy_entries if e.url}.values()),
                "fuzzy-host-path",
                url,
            )
            if result:
    ...
    def clear(self) -> None:
        ...
        self._canonical_urls.clear()
        self._canonical_hosts.clear()
