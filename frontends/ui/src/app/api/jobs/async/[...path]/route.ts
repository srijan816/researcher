...
export async function GET(
  ...
  try {
    ...
    // Non-JSON payloads (e.g. report?format=md markdown, report?format=docx
    // bytes) must pass through untouched — response.json() would corrupt them.
    ...
    if (!contentType.includes('application/json')) {
      ...
      if (contentDisposition) passthroughHeaders['Content-Disposition'] = contentDisposition
