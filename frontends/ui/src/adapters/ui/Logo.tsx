 ...
 *   (off-white at rest, accent blue on hover in the nav rail).
...
const WORDMARK_PATHS: Array<{ d: string; transform: string }> = [
  {
  ...
  if (isMarkOnly) {
    return (
      ...
      >
        ...
        <path d={ALPHA_PATH} fill="currentColor" transform="translate(120.94,84) scale(0.02148,-0.02148)" />
      </svg>
    )
  }
  ...
  return (
    ...
    >
      ...
      {WORDMARK_PATHS.map((path, index) => (
        <path key={index} d={path.d} fill="currentColor" transform={path.transform} />
      ))}
    </svg>
  )
}
