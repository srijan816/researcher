  ...
  const connect = useCallback(
    (jobId: string, bufferReplay = false) => {
      ...
      const flushBuffer = (): void => {
        ...
        const citations = buf.citations.map((c, i) => ({ id: `citation-${i}`, url: c.url, content: c.content, isCited: c.isCited, timestamp: parseEventDate(c.timestamp) }))
      ...
      const client = createDeepResearchClient({
        ...
        callbacks: {
          ...
          onCitationUpdate: (url, content, isCited, timestamp) => {
            if (buf.active) { buf.citations.push({ url, content, isCited: isCited ?? false, timestamp }); return }
            if (!isOwnerActive(jobId)) return
