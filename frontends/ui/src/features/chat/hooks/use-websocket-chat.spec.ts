...
describe('useWebSocketChat', () => {
  ...
  test('sendMessage sends via WebSocket when connected', () => {
    ...
    expect(mockWsClient.sendMessage).toHaveBeenCalledWith('Hello', expect.any(Array), 'deeper', 'aiq')
  ...
  test('sendMessage does not add knowledge_layer when no files uploaded', async () => {
    ...
    expect(mockWsClient.sendMessage).toHaveBeenCalledWith('Hello', ['web', 'docs'], 'deeper', 'aiq')
  })
  ...
  test('sendMessage adds knowledge_layer when files are uploaded', async () => {
    ...
    expect(mockWsClient.sendMessage).toHaveBeenCalledWith('Hello', ['web', 'docs', 'knowledge_layer'], 'deeper', 'aiq')
  })
  ...
  test('sendMessage adds knowledge_layer when files are ingesting', async () => {
    ...
    expect(mockWsClient.sendMessage).toHaveBeenCalledWith('Hello', ['web', 'knowledge_layer'], 'deep', 'aiq')
  })
  ...
  test('sendMessage queued before connect is sent exactly once across reconnects', () => {
    // WebSocket not connected yet: sendMessage creates a client and defers the
    // send until the first 'connected' status.
    mockWsClient.isConnected.mockReturnValue(false)
    ...
    // First successful connection delivers the message.
    mockWsClient.isConnected.mockReturnValue(true)
    act(() => {
    ...
    // An automatic reconnect fires 'connected' again (after a backend restart
    // or network blip). The queued message must NOT be re-sent: re-sending
    // re-runs the chat workflow and spawns a duplicate research job.
    act(() => {
