...
class JobMetrics:
    """Best-effort per-job telemetry accumulator.

    Collects per-tool call counts, search/document/scrape/cache counters,
    error counts, and rough phase timings during a run. The runner persists
    the summary as a ``job.metrics`` event just before the terminal event.
    Thread-safe because parallel researcher lanes share one callback handler.
    All recording is wrapped defensively — metrics must never fail a job.
    """

    CACHE_HIT_MARKER = "<cached>true</cached>"
    ...
    def __init__(self, clock: Any | None = None):
        ...
        self.scrape_artifacts: int = 0
        self.cache_hits: int = 0
        self.llm_calls: int = 0
        self.errors: dict[str, int] = {}
        self.plan_written_at: float | None = None
        self.first_search_started_at: float | None = None
        self.last_search_ended_at: float | None = None

    def record_tool_start(self, tool_name: str, *, is_search: bool = False) -> None:
        try:
            now = self._clock()
            with self._lock:
                ...
                if is_search:
                    self.search_calls += 1
                    if self.first_search_started_at is None:
                        self.first_search_started_at = now
        except Exception:  # noqa: BLE001 - metrics are best-effort
            logger.debug("JobMetrics.record_tool_start failed", exc_info=True)
    ...
    def record_tool_end(
        ...
        *,
        ...
        sources_seen: int = 0,
        scrape_artifacts: int = 0,
    ) -> None:
        try:
            now = self._clock()
            with self._lock:
                self.documents_seen += max(0, int(sources_seen))
                self.scrape_artifacts += max(0, int(scrape_artifacts))
                if output_text and self.CACHE_HIT_MARKER in output_text:
                    self.cache_hits += 1
                if is_search:
                    self.last_search_ended_at = now
                if str(tool_name or "").lower() == "write_plan" and self.plan_written_at is None:
                    self.plan_written_at = now
        except Exception:  # noqa: BLE001 - metrics are best-effort
            logger.debug("JobMetrics.record_tool_end failed", exc_info=True)
    ...
    def record_llm_call(self) -> None:
        try:
            ...
            logger.debug("JobMetrics.record_llm_call failed", exc_info=True)

    def record_error(self, error_type: str) -> None:
        try:
            with self._lock:
                key = str(error_type or "Unknown")
                self.errors[key] = self.errors.get(key, 0) + 1
        except Exception:  # noqa: BLE001
            logger.debug("JobMetrics.record_error failed", exc_info=True)

    def summary(self) -> dict[str, Any]:
        """Build the summary dict persisted in the ``job.metrics`` event."""
        with self._lock:
            now = self._clock()
            research_start = self.plan_written_at or self.first_search_started_at
            planning_seconds = round(research_start - self.started_at, 3) if research_start else None
            researching_seconds = (
                round(self.last_search_ended_at - research_start, 3)
                if research_start and self.last_search_ended_at and self.last_search_ended_at >= research_start
                else None
            )
            synthesis_seconds = (
                round(now - self.last_search_ended_at, 3) if self.last_search_ended_at is not None else None
            )
            return {
                "schema_version": "1.0",
                ...
                "scrape_artifacts": self.scrape_artifacts,
                ...
                "phases": {
                    ...
                    "researching_seconds": researching_seconds,
                    "synthesis_seconds": synthesis_seconds,
                    "total_seconds": round(now - self.started_at, 3),
                },
            }
...
class AgentEventCallback(BaseCallbackHandler):
    ...
    def on_llm_error(self, error: BaseException, **kwargs) -> None:
        """Count LLM failures in job metrics (no UI event; retries are surfaced via chains)."""
        self.metrics.record_error(type(error).__name__)

    def on_tool_error(self, error: BaseException, **kwargs) -> None:
        """Count tool failures in job metrics."""
        self.metrics.record_error(type(error).__name__)
    ...
    def on_tool_start(self, serialized: dict | None, input_str: str, **kwargs) -> None:
        ...
        self.metrics.record_tool_start(tool_name, is_search=self._is_search_tool(tool_name))
    ...
    def on_tool_end(self, output: str, **kwargs) -> None:
        ...
        metrics_sources_seen = 0
        metrics_scrape_artifacts = 0
        ...
        if self._is_search_tool(tool_name) and output:
            ...
            metrics_sources_seen = len(sources)
            ...
            metrics_scrape_artifacts = len(artifacts)
        ...
        self.metrics.record_tool_end(
            tool_name,
            str(output) if output else None,
            is_search=self._is_search_tool(tool_name),
            sources_seen=metrics_sources_seen,
            scrape_artifacts=metrics_scrape_artifacts,
        )
